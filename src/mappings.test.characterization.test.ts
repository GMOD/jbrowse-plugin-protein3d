...
test('structureSeqVsTranscriptSeqMap pins the residue<->residue maps', () => {
  ...
  expect(structureSeqToTranscriptSeqPosition).toEqual({ 0: 0, 1: 1, 3: 2, 4: 4 })
  expect(transcriptSeqToStructureSeqPosition).toEqual({ 0: 0, 1: 1, 2: 3, 4: 4 })
})

test('structurePositionToAlignmentMap pins structurePos -> alignment column', () => {
  expect(structurePositionToAlignmentMap(PA)).toEqual({ 0: 0, 1: 1, 2: 2, 3: 3, 4: 5 })
})

test('transcriptPositionToAlignmentMap pins transcriptPos -> alignment column', () => {
  expect(transcriptPositionToAlignmentMap(PA)).toEqual({ 0: 0, 1: 1, 2: 3, 3: 4, 4: 5 })
})
