...
**The externals are what EVERY host re-exports, not what core does.**
`esbuild.mjs` externalizes a path only when the installed `@jbrowse/core`
ReExports list and every host in `scripts/host-reexports.json` (4.0.0, 4.3.0,
5.0.0-beta.8, regenerated by `pnpm update-host-reexports`) carry it, and it
always bundles `@mui/material/SvgIcon`, whose shape differs between MUI 7 and
MUI 9 hosts.

Every host rather than just the oldest, because the list moves both ways. A path
only a new core lists is undefined on an old host; a path an old core had and a
new one **dropped** is undefined on the new one, which is how
`defaultCodonTable` leaving the `@jbrowse/core/util` barrel took out msaview and
protein3d. Measured 2026-09-14: 4.0.0 re-exports five renderer base classes
5.0.0-beta.8 does not. This plugin imports none of them, so widening the rule
changed the bundle by zero bytes — it is a guard against the next import, not a
fix. Only booting on `main` would have caught such a path otherwise, and only if
its module evaluates eagerly. `pnpm check-host-externals` greps the built output
to confirm. Before this guard, bumping to `@mui/icons-material` 9 and
`@jbrowse/core` 5 produced a bundle that passed tsc, eslint and every unit test,
and error-paged v4.0.0, v4.3.0 and `latest` with
`createSvgIcon is not a function` — the msaview 2.7.0 outage again.
`host-compat:candidate` caught it. Bundling SvgIcon pulls in `@emotion/styled`,
which is why that devDependency exists.
