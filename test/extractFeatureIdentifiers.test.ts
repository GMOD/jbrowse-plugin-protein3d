...
const mockUseAlphaFoldData = useAlphaFoldData as vi.MockedFunction<typeof useAlphaFoldData>;
const mockUseAlphaFoldSequenceSearch = useAlphaFoldSequenceSearch as vi.MockedFunction<typeof useAlphaFoldSequenceSearch>;
const mockUseIsoformProteinSequences = useIsoformProteinSequences as vi.MockedFunction<typeof useIsoformProteinSequences>;
const mockUseUniProtSearch = useUniProtSearch as vi.MockedFunction<typeof useUniProtSearch>;
const mockGetSearchDescription = getSearchDescription as vi.MockedFunction<typeof getSearchDescription>;
...
describe('useAlphaFoldDBSearch', () => {
  ...
  beforeEach(() => {
    ...
    mockUseAlphaFoldData.mockReturnValue({
      predictions: null, isLoading: false, error: null, selectedEntryIndex: 0, setSelectedEntryIndex: vi.fn(), url: null, confidenceUrl: null, structureSequence: null
    });
    mockUseAlphaFoldSequenceSearch.mockReturnValue({
      uniprotId: null, cifUrl: null, plddtDocUrl: null, structureSequence: null, isLoading: false, error: null
    });
    mockUseIsoformProteinSequences.mockReturnValue({
      isoformSequences: {}, isLoading: false, error: null
    });
  ...
  it('should initialize selectedQueryId with the first recognized ID if available', () => {
    ...
    const { result } = renderHook(() => useAlphaFoldDBSearch({ feature: mockFeature, view: mockView }));
  ...
  it('should compute autoTranscriptId correctly directly', () => {
    const mockTranscript1 = new SimpleFeature({ id: 'transcript1', seq: 'MALS...' });
    const mockTranscript2 = new SimpleFeature({ id: 'transcript2', seq: 'MALS....*' });
    ...
    (mockGetTranscriptFeatures as vi.Mock).mockReturnValue(mockTranscriptOptions);
    mockUseIsoformProteinSequences.mockReturnValue({
      isoformSequences: mockIsoformSequences, isLoading: false, error: null
    });
    mockUseAlphaFoldData.mockReturnValue({
    ...
    (mockSelectBestTranscript as vi.Mock).mockReturnValue({ id: () => mockSelectedTranscriptId });
...
describe('extractFeatureIdentifiers', () => {
  ...
  beforeEach(() => {
    ...
    (mockGetTranscriptFeatures as vi.Mock).mockImplementation((f) => {
      if (f.get('type') === 'gene') {
        ...
        return f.get('subfeatures')?.filter((sf: Feature) => sf.get('type') === 'transcript' || sf.get('type') === 'mRNA') || [];
      }
      return [];
    });
  ...
  it('should extract identifiers from the first transcript subfeature if the input is a gene with transcripts', () => {
    ...
    expect(identifiers.recognizedIds).toEqual(expect.arrayContaining([
      'ENST00000123456',
      'HGNC:5678',
      'NM_001310462.2'
    ]));
